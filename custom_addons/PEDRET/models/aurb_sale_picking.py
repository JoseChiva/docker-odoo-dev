...
class AurenSalesPicking(models.Model):
    ...
    sale_order_line_ids = fields.Many2many(
        "sale.order.line", domain="[('state','=','llibreta'),('pick_id','=',False)]")

    pending_line = fields.Float(
        string="Líneas pendientes", compute="_compute_cost_product")

    sale_order_lines_count = fields.Integer(
        string='Número de Líneas',
        compute='_compute_sale_order_lines_count',
        store=True  # Opcional: si quieres que se almacene en la BD
    )
    ...
    def _compute_sale_order_lines_count(self):
        for record in self:
            record.sale_order_lines_count = len(record.sale_order_line_ids)
